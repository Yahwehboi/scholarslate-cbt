...
function StudentsTable({ students }: { students: StudentRecord[] }) {
  return (
      ...
      <div className="main-content flex flex-col min-h-screen">
        <header className="sticky top-0 z-20 flex items-center justify-between px-6 md:px-8 h-16"
          ...
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg font-bold" style={{ fontFamily: 'Manrope,sans-serif', color: '#191c1d' }}>Student Registry</h2>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 p-5 md:p-8 max-w-screen-xl mx-auto w-full">
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2" style={{ fontFamily: 'Manrope,sans-serif', color: '#191c1d' }}>Registered Students</h1>
            <p className="text-base" style={{ color: '#3d4a3d' }}>
              Students shown here can sign in using only their Student ID.
            </p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-7">
            <div className="p-6 rounded-2xl" style={{ backgroundColor: '#ffffff', boxShadow: '0 2px 12px rgba(25,28,29,0.07)' }}>
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#6d7b6c' }}>Total Students</p>
              <p className="text-4xl font-extrabold mt-2" style={{ fontFamily: 'Manrope,sans-serif', color: '#006e2f' }}>{students.length}</p>
            </div>
            <div className="md:col-span-2 p-6 rounded-2xl" style={{ backgroundColor: '#ffffff', boxShadow: '0 2px 12px rgba(25,28,29,0.07)' }}>
              <label className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#6d7b6c' }}>Search Students</label>
              <input
                type="text"
                value={query}
                onChange={e => setQuery(e.target.value)}
                placeholder="Search by ID, name, or class"
                style={{ width: '100%', marginTop: '10px', padding: '10px 12px', border: '1px solid #e7e8e9', borderRadius: '0.75rem', outline: 'none' }}
              />
            </div>
          </div>
          ...
          {filteredStudents.length > 0 ? (
            ...
            <div className="p-10 rounded-2xl text-center" style={{ backgroundColor: '#ffffff', boxShadow: '0 4px 20px rgba(25,28,29,0.07)' }}>
              <p className="text-lg font-bold" style={{ color: '#191c1d' }}>No students found</p>
              <p className="text-sm mt-1" style={{ color: '#6d7b6c' }}>Try a different search or register students from Upload Questions.</p>
            </div>
          )}
        </main>
      </div>
