...
export default function ResultPage(){
  ...
  useEffect(() => {
    ...
    api.students.getResult(resultId)
  ...
  const score      = result?.scorePct ?? 0
  const subject    = result?.subjectName ?? 'Unknown Subject'
  
  // Calculate time used roughly if available
  let timeUsed = '—'
    ...
    <div className="min-h-screen" style={{backgroundColor:'#f8f9fa'}}>
      ...
      <div className="main-content flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-5 md:p-8 max-w-4xl mx-auto w-full print-area">
          {loading ? (
            <div className="flex justify-center items-center py-20 text-[#6d7b6c]">Loading result details...</div>
          ) : error ? (
            <div className="text-center py-20 text-[#9e4036] font-bold">{error}</div>
          ) : (
            <>
              <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="mb-6 no-print">
                ...
                <p className="text-xs uppercase tracking-widest font-medium" style={{color:'#6d7b6c'}}>Session: {new Date().getFullYear()}/{new Date().getFullYear()+1} • {subject}</p>
              </motion.div>
          ...
          <p className="text-center text-xs uppercase tracking-widest" style={{color:'#bccbb9'}}>
            Generated by ScholarSlate CBT • Ref: {result?.id.slice(0, 8).toUpperCase() ?? 'N/A'}
          </p>
            </>
          )}
        </main>
